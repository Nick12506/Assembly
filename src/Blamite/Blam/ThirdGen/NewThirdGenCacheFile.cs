...
namespace Blamite.Blam.ThirdGen
{
	...
	public class NewThirdGenCacheFile : ICacheFile
	{
		...
		private NewThirdGenTagTable _tags;
		private ThirdGenSimulationDefinitionTable _simulationDefinitions;

		public NewThirdGenCacheFile(IReader reader, EngineDescription buildInfo, string buildString)
		{
		...
		public NewThirdGenHeader FullHeader
		{
			get { return _header; }
		}
		...
		{
			get { return EngineType.ThirdGenMCC; }
		}
		...
		private void LoadHeader(IReader reader, string buildString)
		{
			...
			_header = new NewThirdGenHeader(values, _buildInfo, buildString, _segmenter);
		}
		...
		private void LoadTags(IReader reader)
		{
			...
			{
				_tags = new NewThirdGenTagTable();
				return;
			}

			_tags = new NewThirdGenTagTable(reader, _header.IndexHeaderLocation, _header.MetaArea, Allocator, _buildInfo);
		...
		private void WriteHeader(IWriter writer)
		{
			...
			// Right now we don't want to touch the header
			// TODO implement header touching
			//StructureValueCollection values = _header.Serialize(null);
			//writer.SeekTo(0);
